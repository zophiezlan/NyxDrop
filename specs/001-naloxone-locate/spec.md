...
| `/me` | Map with the My Places sheet open. Deep-linkable; closing the sheet returns to `/`. Not a separate page. |
...
### 6.4 Step 3 — Which barriers? (only for `success_but`, `denied`, and optionally `out_of_stock`)

Multi-select checklist. The shown rows depend on the report type — the
vocabulary is shared but not every barrier is meaningful for every type.

| Barrier | `success_but` | `out_of_stock` | `denied` |
|---|---|---|---|
| `id_required` — Asked for ID | ✓ | — | ✓ |
| `medicare_required` — Wanted Medicare card | ✓ | — | ✓ |
| `prescription_required` — Wanted a prescription | ✓ | — | ✓ |
| `cost_involved` — Charged me (reveals `$ amount`) | ✓ | — | — |
...
Selection rules by report type:

- `success`: barriers MUST be empty. Step 3 is skipped.
- `success_but`: at least one barrier from the `success_but` column.
- `out_of_stock`: barriers MUST be empty OR contain only barriers from the
  `out_of_stock` column. Step 3 is optional — most stock-out reports omit it.
- `denied`: at least one barrier from the `denied` column. `wrong_form_only`,
  `cost_involved`, `long_wait`, and `limited_hours` are not surfaced (you didn't
  get anything, so they're not the salient story).

The "Charged me" row reveals an optional dollar-amount text field; if filled,
it's stored on the report. Server validates the same matrix on submit
(`contracts.md` Reports section).
...
Implemented as a sheet that slides over the map. The path `/me` is a deep-link
variant of `/` — the map renders behind, the My Places sheet auto-opens on top.
Closing the sheet returns the URL to `/`. There is no separate `/me` page.
