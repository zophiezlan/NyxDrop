...
Validation rules (enforced server-side via Zod). The barrier vocabulary varies
by `reportType` — see `spec.md` §6.4 for the full matrix:

- `barriers` must be empty for `success`
- `barriers` must contain ≥1 item for `success_but` and `denied`
- `barriers` may be empty or contain only the `out_of_stock`-eligible subset
  (`wrong_form_only`, `staff_unsure`, `staff_rude`, `limited_hours`) for
  `out_of_stock`
- `barriers` for `denied` must NOT include `cost_involved`, `wrong_form_only`,
  `long_wait`, or `limited_hours` (a denied user did not get a transaction;
  these barriers are not the salient story)
- `notes` ≤ 500 chars
- `costAmount` ≤ 1000, only valid if `cost_involved` in `barriers`, only
  permitted on `success_but`
